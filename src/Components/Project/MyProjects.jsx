...
const MyProjects = () => {
  ...
  const caseStudyData = [
    {
      ...
      tools:
        "Instagram Optimization • Audience Targeting • Content Strategy",
...

    {
      ...
      tools:
        "Instagram Insights • Analytics Tracking • KPI Monitoring",
...

    {
      ...
      tools:
        "Brand Strategy • Visual Optimization • Content Planning",
  ...
  const Lightbox = () => {
    ...
    return (
      ...
      >
        ...
        <div
          className="relative w-full max-w-5xl bg-[#0a0a0a] rounded-3xl border border-white/10 overflow-hidden shadow-2xl"
          onClick={(e) => e.stopPropagation()}
        >

          {/* Close Button */}
          <button
            className="absolute top-5 right-5 z-[1001] bg-white/20 hover:bg-white/40 text-white w-10 h-10 rounded-full flex items-center justify-center transition-all"
            onClick={() => setSelectedItem(null)}
          >
            <X size={20} />
          </button>
  ...
  return (
    ...
    <section
      id="projects"
      className="bg-[#0a0a0a] h-full py-24 px-6 relative overflow-hidden"
    >
          ...
          {caseStudyData.map((item, i) => (
            ...
            >
              ...
              <div className="relative w-full aspect-[4/3] rounded-[2rem] overflow-hidden mb-6">
                ...
                <div
                  className={`absolute top-4 left-4 ${item.color} text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-full text-white shadow-lg`}
                >
                  {item.tag}
                </div>
              </div>
              ...
              <div className="px-2 flex-grow">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  ...
                  {item.stats.map((stat, idx) => (
                    ...
                    <span
                      key={idx}
                      className="bg-white/5 border border-white/10 text-gray-400 text-[9px] font-bold uppercase tracking-tighter px-3 py-1.5 rounded-lg group-hover:text-white transition-colors"
                    >
                      {stat}
                    </span>
                  ))}
